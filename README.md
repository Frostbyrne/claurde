...
```
# api_key = sk-ant-...          # inline (or, better:)
...
- **Verdicts are cached by commit.** A verdict is keyed on the package-base git
  commit hash and the model used. Re-running on an unchanged package is free (no
  API call); any content change is a new commit, so it's always re-reviewed.
  Cache lives under `$XDG_CACHE_HOME/claurde/verdicts/`; bypass with `--no-cache`.
...
- **Upstream review covers the source tree, not a fully materialized dependency
  ...
  roadmap.
- **VCS sources without a pinned ref can drift.** A `git+` source pointing at a
  branch (rather than a tag/commit) can change content without the AUR
  package-base commit changing, so a cached verdict could go stale for it. Use
  `--no-cache` if you're reviewing a moving git source.
