...
/// If the config file holds an inline `api_key`, ensure it isn't group/world
/// readable. The onboarding key file is created 0600, but a user who pasted
/// `api_key = sk-ant-...` into the config by hand likely left it at the editor
/// default (often 0644). Tighten in place and warn. Best-effort.
fn protect_inline_key() {
    let Some(path) = config_path() else { return };
    let has_inline = read_config_file()
        .into_iter()
        .any(|(k, v)| k == "api_key" && !v.is_empty());
    if !has_inline {
        return;
    }
    if let Ok(meta) = std::fs::metadata(&path) {
        if meta.permissions().mode() & 0o077 != 0 {
            ...
            eprintln!(
                "note: tightened {} to 0600 — it holds an inline api_key and was group/world-readable.",
                path.display()
            );
        }
    }
